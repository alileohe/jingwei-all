...
start()
    {
        echo "========================================================================="
        echo ""
        echo "  JingWei server Startup Environment"
        echo ""
        ...
        if [ -f "${PID_FILE}" ]; then
            ...
            if [ "" != "${PID_NUM}" ]; then
                RUN__PID=`ps aux | grep -v "grep" | grep "jingwei.type=server" | awk '{print $2}' | sed -n '1P' | grep ${PID_NUM}`
                if [ "" != "${RUN__PID}" ]; then
                    echo "Jingwei Server Already Running..............!"
                    exit 1;
                fi
        ...
        #Start Java Process
        ${JAVA_HOME}/bin/java ${JAVA_OPTS}  -classpath ${CONF_DIR}:${CLASSPATH}:. com.taobao.jingwei.server.JingweiServerMain ${BASE_DIR}/conf/server.ini >> ${START_LOG} 2>&1 &
